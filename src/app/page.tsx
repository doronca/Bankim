...
export default function DashboardPage() {
  ...
  const zeroCardsCount = cardsInScope.filter((c) => c.pendingAmount === 0).length;
  const displayedCards = hideZeroCards ? cardsInScope.filter((c) => c.pendingAmount !== 0) : cardsInScope;
  const dateFmt = (d: string) =>
    new Date(d).toLocaleDateString(locale === "he" ? "he-IL" : "en-US", { day: "numeric", month: "short" });

  const sectionDefs: { id: string; title: string; node: React.ReactNode }[] = [
    cardsInScope.length > 0 && {
      id: "cards",
      title: t.accountTypeCreditCard,
      node: (
        <div className="flex flex-col gap-2">
          {zeroCardsCount > 0 && (
            <label className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 self-start">
              <input type="checkbox" checked={hideZeroCards} onChange={(e) => setHideZeroCards(e.target.checked)} />
              {locale === "he"
                ? `הסתר כרטיסים ללא חיוב קרוב (${zeroCardsCount})`
                : `Hide cards with no upcoming charge (${zeroCardsCount})`}
            </label>
          )}
    ...
    {
      id: "ask",
      title: locale === "he" ? "שאל" : "Ask",
      node: <AskBox entity={entity} locale={locale} />,
    },
    {
      id: "insights",
      title: locale === "he" ? "תובנות" : "Insights",
      node: <InsightsPanel entity={entity} locale={locale} />,
    },
    {
      id: "tasks",
      title: locale === "he" ? "משימות" : "Tasks",
      node: <TasksPanel entity={entity} locale={locale} />,
    },
    {
      id: "networth",
      title: locale === "he" ? "שווי נטו ומדדים" : "Net worth & metrics",
      node: (
    ...
    {
      id: "cardSpend",
      title: locale === "he" ? "הוצאות כרטיסים" : "Card spend",
      node: (
    ...
    {
      id: "change",
      title: locale === "he" ? "שינויים מול חודש קודם" : "Changes vs. last month",
      node: (
    ...
    {
      id: "categoryMerchants",
      title: locale === "he" ? "קטגוריות ובתי עסק מובילים" : "Categories & top merchants",
      node: (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={CARD}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">{t.category}</h3>
    ...
    {
      id: "biggest",
      title: locale === "he" ? "התנועות הגדולות ביותר (חודש נוכחי)" : "Biggest transactions (this month)",
      node: (
        <div className={CARD}>
          <div className="flex flex-col gap-2">
  ...
  ].filter(Boolean) as { id: string; title: string; node: React.ReactNode }[];

  const allIds = sectionDefs.map((s) => s.id);
  const orderedIds = [
    ...dashboardSectionOrder.filter((id) => allIds.includes(id)),
    ...allIds.filter((id) => !dashboardSectionOrder.includes(id)),
  ];
  const sectionsById = new Map(sectionDefs.map((s) => [s.id, s]));
  const visibleOrderedSections = orderedIds
    .map((id) => sectionsById.get(id)!)
    .filter((s) => !hiddenDashboardSections.includes(s.id));
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <DashboardCustomizer
            sections={sectionDefs.map(({ id, title }) => ({ id, title }))}
            locale={locale}
            onExpandAll={() => setCollapsedDashboardSections([])}
            onCollapseAll={() => setCollapsedDashboardSections(allIds)}
          />
        </div>
      </div>
      ...
      {visibleOrderedSections.map((s) => (
        <section key={s.id}>
          <DashboardSectionHeader
            title={s.title}
            locale={locale}
            collapsed={collapsedDashboardSections.includes(s.id)}
            onToggleCollapse={() =>
              setCollapsedDashboardSections(
                collapsedDashboardSections.includes(s.id)
                  ? collapsedDashboardSections.filter((x) => x !== s.id)
                  : [...collapsedDashboardSections, s.id]
              )
            }
            onHide={() => setHiddenDashboardSections([...hiddenDashboardSections, s.id])}
            isDragging={draggedId === s.id}
            isDropTarget={dragOverId === s.id && draggedId !== s.id}
          ...
          />
          {!collapsedDashboardSections.includes(s.id) && s.node}
        </section>
      ))}
    </div>
  );
